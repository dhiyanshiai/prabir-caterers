      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="grid gap-12 grid-cols-1 md:grid-cols-3">
          {services.map((service, idx) => (
            <div
              key={service.id}
              className={`card-premium group relative cursor-pointer tilt-card cursor-glow scroll-reveal-scale stagger-${idx + 1}`}
          ...
          <div className="bg-white w-full max-w-3xl rounded-3xl overflow-hidden relative z-10 animate-slide-up shadow-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="h-32 sm:h-40 relative">
              ...
              <button
                ...
                className="absolute top-2 right-2 bg-white/20 hover:bg-white text-white hover:text-royal p-2 rounded-full transition-all backdrop-blur-md"
              >
                <X size={20} />
              </button>
              <div className="absolute bottom-3 left-4 sm:left-6 text-white">
                <h2 className="text-2xl sm:text-3xl font-serif font-bold text-white">{selectedService.title}</h2>
              </div>
            </div>
